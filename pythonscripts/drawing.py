...
# Extracts the IDs and fitness values of the POIs from a JSONL file.
def extract_id_mu(file_path):
    IDs = []
    fitnesses = []
    with open(file_path, 'r') as file:
...
# Extracts the IDs, fitness values, and swarm scores of the POIs from a JSONL file.
def extract_id_mu_swarmscore(file_path):
    IDs = []
    fitnesses = []
    swarmscores = []
...
# Draws the titles of the POIs, creates a dictionary with keys as the POI Id, and the value as the title of the POI
# Returns the title of the POIs in the order of the IDs
def draw_title(IDs, filepath):
    id_to_title = {}
...
# Drawing process from the old bucket. 
def draw_from_old_bucket(N):
    IDs, fitnesses = extract_id_mu('data/old_bucket.jsonl')
    picked_IDs = []
...
def draw_from_new_bucket(N):
    IDs, fitnesses, swarm_scores = extract_id_mu_swarmscore('data/new_bucket.jsonl')
    sorted_IDs = [x for _, x in sorted(zip(swarm_scores, IDs), reverse=True)]
        ...
        with open(filepath, 'r') as file:
            for line in file:
                try:
                    poi = json.loads(line)
                    if poi['id'] in IDs:
                        id_to_oneliner[poi['id']] = poi['one_liner']
                        id_to_description[poi['id']] = poi['description']
                except json.JSONDecodeError:
                    print(f"Skipping malformed line: {line.strip()}")

    oneliners = [id_to_oneliner[id] for id in IDs if id in id_to_oneliner]
    descriptions = [id_to_description[id] for id in IDs if id in id_to_description]
